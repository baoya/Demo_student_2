		...
		setContentView(R.layout.register);
	     ...
	     re_register.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(username.length() != 0 && userpass.length() < 6 && userpass_2.length() < 6)
				{
					Toast.makeText(zhuce.this, "密码长度过短，请输入大于登录6个数！", Toast.LENGTH_SHORT).show();
				}
				else if(username.length() != 0 && userpass.length() >= 6 && userpass_2.length() >= 6)
				{
					...
					{
						Toast.makeText(zhuce.this, "你两次输入的密码不一样，请重新输入！", Toast.LENGTH_SHORT).show();
					}
...
//		 BufferedWriter writer = null;
//		 try {
//			 out = openFileOutput("userpass", Context.MODE_PRIVATE);
//			 writer = new BufferedWriter(new OutputStreamWriter(out));
